...
def box_plot(
    ...
    show_detail=False,    # show mean and median at x-ticks
    ...
    show_detail = False if show_detail is None else show_detail
    ...
    for i in range(ax_num):
        ...
        if test!=None and (condition>1 or len(columns)>1):
            if test=='U-test':
                add_test = True
        ...
        if show_detail:
            ...
            new_xticks = ["{}\navg:{}\nmedian:{}".format(xtick, mean, median) for (xtick,mean,median) in zip(xticks,mean_list,median_list)]
            xticks_dict = dict(zip(xticks, new_xticks))
            ax.set_xticklabels([ xticks_dict[xtick.get_text()] for xtick in ax.get_xticklabels() ])
